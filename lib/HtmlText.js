...
function styleForTag(tagName) {
  ...
  console.log("style for tag: ", tagName, style)
  return style
}
...
function processNode(node, parentKey) {
  ...
  if (isBlockElement(nodeName)) {
    ...
    node.childNodes.forEach((childNode, index) => {
      ...
      if (isInlineElement(childNode.nodeName) || isText(childNode.nodeName)) {
        ...
        if (lastInlineNodes.length > 0) {
          children.push(<Text key={`${key}_${index}_${childNode.nodeName}`}>{lastInlineNodes}</Text>)  
          lastInlineNodes = []
        }
        children.push(child)
    ...
    if (lastInlineNodes.length > 0) {
      children.push((<Text>{lastInlineNodes}</Text>))  
    }
